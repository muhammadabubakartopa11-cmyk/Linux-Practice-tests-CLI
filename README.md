# Linux basic CLI Test#1
Files details:
I have tested my basic linux skills in these files.
## Here is a list of commands which I tested in following files:
| File name | Commands |
| :--- | :--- |
| `students.txt`            `tail`, `head`, `nano`, `grep` (-n, -i, -v), `ls` |
| `students2.txt`           `sort` (-r, -n) `nano`, `uniq` (-c, -d, -u), `wc` (-l, -m, -w), `ls` |
| `cities.txt`              `cut`, `man`, `rm`, `ls` |

### Note:
I can use many other basic commands which I have not tested yet.
For example: `pwd`, `touch`, `find`, `whereis`, `which`, `cat`, `cd`, `mkdir`, `rm -r`, `rmdir`, and many more. 
